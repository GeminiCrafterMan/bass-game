; ===========================================================================
; Uncompressed player graphics
; ===========================================================================

ArtUnc_Bass:			binclude "Objects/Player Characters/Bass/Uncompressed Art/Bass.bin"
	even
ArtUnc_CopyRobot:		binclude "Objects/Player Characters/Copy Robot/Uncompressed Art/Copy Robot.bin"
	even
ArtUnc_DashDust:		binclude "Objects/Spin Dust/Uncompressed Art/Dash Dust.bin"
	even
ArtUnc_SplashDrown:		binclude "Objects/Spin Dust/Uncompressed Art/Splash Drown.bin"
	even